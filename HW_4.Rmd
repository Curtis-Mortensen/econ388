...
```{r}
expendA <- data$expendA
expendB <- data$expendB
prtyA <- data$prtystrA
voteA <- data$voteA
...
m3 <- lm(salary~rank+LSAT+llibvol+lcost)
#m22 <- lm(data$salary~data$LSAT+log(data$libvol)+log(data$cost)+data$rank)
...
```{r}
m32 <- lm(data$lavgsal~data$bs+data$lenrol+data$lstaff)
summary(m32)
```
...
```{r}
m33 <- lm(formula = data$lavgsal ~ data$bs + data$lenrol + data$lstaff + data$lunch)
summary(m33)
```
